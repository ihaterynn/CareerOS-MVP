...
drop policy if exists "Users read own employer memberships" on public.employer_memberships;
create policy "Users read own employer memberships"
on public.employer_memberships for select to authenticated
using (user_id = auth.uid());

drop policy if exists "Employer members read CV batches" on public.cv_ingestion_batches;
...
drop policy if exists "Employer members read CV records" on public.cv_ingestion_records;
create policy "Employer members read CV records"
on public.cv_ingestion_records for select to authenticated
using (public.is_employer_member(employer_id));

drop policy if exists "Employer members read role matches" on public.cv_role_matches;
create policy "Employer members read role matches"
on public.cv_role_matches for select to authenticated
using (public.is_employer_member(employer_id));

drop policy if exists "Employer members update role matches" on public.cv_role_matches;
...
drop policy if exists "Employer members read review events" on public.cv_review_events;
create policy "Employer members read review events"
on public.cv_review_events for select to authenticated
using (public.is_employer_member(employer_id));

drop policy if exists "Employer members create review events" on public.cv_review_events;
